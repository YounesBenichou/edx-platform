...
from openedx.core.djangoapps.password_policy import (
    compliance as password_policy_compliance,
)
from openedx.core.djangoapps.password_policy.forms import (
    PasswordPolicyAwareAdminAuthForm,
)
...
urlpatterns = oauth2_urlpatterns + [
    ...
    path(
        "transcripts/upload",
        contentstore_views.upload_transcripts,
    ...
    path(
        "transcripts/check",
        contentstore_views.check_transcripts,
        name="check_transcripts",
    ),
    path(
        "transcripts/choose",
        contentstore_views.choose_transcripts,
        name="choose_transcripts",
    ),
    path(
        "transcripts/replace",
        contentstore_views.replace_transcripts,
        name="replace_transcripts",
    ),
    path(
        "transcripts/rename",
        contentstore_views.rename_transcripts,
    ...
    path(
        "",
        include("openedx.core.djangoapps.xblock.rest_api.urls", namespace="xblock_api"),
    ),
    ...
    path(
        "update_lang/",
        include("openedx.core.djangoapps.dark_lang.urls", namespace="dark_lang"),
    ),
    ...
    path(
        ...
        contentstore_views.request_course_creator,
        name="request_course_creator",
    ),
    re_path(
        rf"^course_team/{COURSELIKE_KEY_PATTERN}(?:/(?P<email>.+))?$",
        contentstore_views.course_team_handler,
        name="course_team_handler",
    ),
    re_path(
        rf"^course_info/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.course_info_handler,
        name="course_info_handler",
    ),
    re_path(
        rf"^course_info_update/{settings.COURSE_KEY_PATTERN}/(?P<provided_id>\d+)?$",
        contentstore_views.course_info_update_handler,
        name="course_info_update_handler",
    ),
    re_path(r"^home/?$", contentstore_views.course_listing, name="home"),
    re_path(
        r"^home_library/?$", contentstore_views.library_listing, name="home_library"
    ),
    re_path(
        rf"^course/{settings.COURSE_KEY_PATTERN}/search_reindex?$",
        contentstore_views.course_search_index_handler,
        name="course_search_index_handler",
    ),
    re_path(
        rf"^course/{settings.COURSE_KEY_PATTERN}?$",
        contentstore_views.course_handler,
        name="course_handler",
    ),
    # path('course/<str:resource_key>/',contentstore_views.course_handler, name='course_handler'),
    re_path(
        rf"^checklists/{settings.COURSE_KEY_PATTERN}?$",
        contentstore_views.checklists_handler,
        name="checklists_handler",
    ),
    re_path(
        rf"^course_notifications/{settings.COURSE_KEY_PATTERN}/(?P<action_state_id>\d+)?$",
        contentstore_views.course_notifications_handler,
        name="course_notifications_handler",
    ),
    re_path(
        rf"^course_rerun/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.course_rerun_handler,
        name="course_rerun_handler",
    ),
    re_path(
        rf"^container/{settings.USAGE_KEY_PATTERN}$",
        contentstore_views.container_handler,
        name="container_handler",
    ),
    re_path(
        rf"^orphan/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.orphan_handler,
        name="orphan_handler",
    ),
    re_path(
        rf"^assets/{settings.COURSE_KEY_PATTERN}/{settings.ASSET_KEY_PATTERN}?$",
        contentstore_views.assets_handler,
        name="assets_handler",
    ),
    re_path(
        rf"^import/{COURSELIKE_KEY_PATTERN}$",
        contentstore_views.import_handler,
        name="import_handler",
    ),
    re_path(
        rf"^import_status/{COURSELIKE_KEY_PATTERN}/(?P<filename>.+)$",
        contentstore_views.import_status_handler,
    ...
    re_path(
        rf"^export/{COURSELIKE_KEY_PATTERN}$",
        contentstore_views.export_handler,
        name="export_handler",
    ),
    re_path(
        rf"^export_output/{COURSELIKE_KEY_PATTERN}$",
        contentstore_views.export_output_handler,
        name="export_output_handler",
    ),
    re_path(
        rf"^export_status/{COURSELIKE_KEY_PATTERN}$",
        contentstore_views.export_status_handler,
        name="export_status_handler",
    ),
    re_path(
        rf"^xblock/outline/{settings.USAGE_KEY_PATTERN}$",
        contentstore_views.xblock_outline_handler,
        name="xblock_outline_handler",
    ),
    re_path(
        rf"^xblock/container/{settings.USAGE_KEY_PATTERN}$",
        contentstore_views.xblock_container_handler,
        name="xblock_container_handler",
    ),
    re_path(
        rf"^xblock/{settings.USAGE_KEY_PATTERN}/(?P<view_name>[^/]+)$",
        contentstore_views.xblock_view_handler,
        name="xblock_view_handler",
    ),
    re_path(
        rf"^xblock/{settings.USAGE_KEY_PATTERN}?$",
        contentstore_views.xblock_handler,
        name="xblock_handler",
    ),
    re_path(
        rf"^tabs/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.tabs_handler,
        name="tabs_handler",
    ),
    re_path(
        rf"^settings/details/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.settings_handler,
        name="settings_handler",
    ),
    re_path(
        rf"^settings/grading/{settings.COURSE_KEY_PATTERN}(/)?(?P<grader_index>\d+)?$",
        contentstore_views.grading_handler,
        name="grading_handler",
    ),
    re_path(
        rf"^settings/advanced/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.advanced_settings_handler,
        name="advanced_settings_handler",
    ),
    re_path(
        rf"^textbooks/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.textbooks_list_handler,
        name="textbooks_list_handler",
    ),
    re_path(
        rf"^textbooks/{settings.COURSE_KEY_PATTERN}/(?P<textbook_id>\d[^/]*)$",
        contentstore_views.textbooks_detail_handler,
        name="textbooks_detail_handler",
    ),
    re_path(
        rf"^videos/{settings.COURSE_KEY_PATTERN}(?:/(?P<edx_video_id>[-\w]+))?$",
        contentstore_views.videos_handler,
        name="videos_handler",
    ),
    re_path(
        rf"^generate_video_upload_link/{settings.COURSE_KEY_PATTERN}",
        contentstore_views.generate_video_upload_link_handler,
        name="generate_video_upload_link",
    ),
    re_path(
        rf"^video_images/{settings.COURSE_KEY_PATTERN}(?:/(?P<edx_video_id>[-\w]+))?$",
        contentstore_views.video_images_handler,
        name="video_images_handler",
    ),
    path(
        "video_images_upload_enabled",
        contentstore_views.video_images_upload_enabled,
        name="video_images_upload_enabled",
    ),
    re_path(
        rf"^transcript_preferences/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.transcript_preferences_handler,
        name="transcript_preferences_handler",
    ),
    re_path(
        rf"^transcript_credentials/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.transcript_credentials_handler,
        name="transcript_credentials_handler",
    ),
    path(
        "transcript_download/",
        contentstore_views.transcript_download_handler,
        name="transcript_download_handler",
    ),
    path(
        "transcript_upload/",
        contentstore_views.transcript_upload_handler,
        name="transcript_upload_handler",
    ),
    re_path(
        r"^transcript_delete/{}(?:/(?P<edx_video_id>[-\w]+))?(?:/(?P<language_code>[^/]*))?$".format(
            settings.COURSE_KEY_PATTERN
        ),
        contentstore_views.transcript_delete_handler,
        name="transcript_delete_handler",
    ),
    path(
        "transcript_upload_api/",
        contentstore_views.transcript_upload_api,
        name="transcript_upload_api",
    ),
    re_path(
        rf"^video_encodings_download/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.video_encodings_download,
        name="video_encodings_download",
    ),
    re_path(
        rf"^group_configurations/{settings.COURSE_KEY_PATTERN}$",
        contentstore_views.group_configurations_list_handler,
        name="group_configurations_list_handler",
    ),
    re_path(
        r"^group_configurations/{}/(?P<group_configuration_id>\d+)(/)?(?P<group_id>\d+)?$".format(
            settings.COURSE_KEY_PATTERN
        ),
        contentstore_views.group_configurations_detail_handler,
...
if not settings.DISABLE_DEPRECATED_SIGNUP_URL:
    ...
    urlpatterns += [
        path(
            "signup",
            contentstore_views.register_redirect_to_lms,
            name="register_redirect_to_lms",
        ),
    ]
...
if settings.FEATURES.get("ENABLE_CONTENT_LIBRARIES"):
    urlpatterns += [
        re_path(
            rf"^library/{LIBRARY_KEY_PATTERN}?$",
            contentstore_views.library_handler,
            name="library_handler",
        ),
        re_path(
            rf"^library/{LIBRARY_KEY_PATTERN}/team/$",
            contentstore_views.manage_library_users,
            name="manage_library_users",
        ),
    ]
...
if toggles.EXPORT_GIT.is_enabled():
    urlpatterns += [
        re_path(
            rf"^export_git/{settings.COURSE_KEY_PATTERN}$",
...
urlpatterns.append(
    path(
        "admin/login/",
        contentstore_views.redirect_to_lms_login_for_admin,
...
if core_toggles.ENTRANCE_EXAMS.is_enabled():
    urlpatterns.append(
        re_path(
            rf"^course/{settings.COURSE_KEY_PATTERN}/entrance_exam/?$",
...
if settings.FEATURES.get("CERTIFICATES_HTML_VIEW"):
    ...
    urlpatterns += [
        re_path(
            rf"^certificates/activation/{settings.COURSE_KEY_PATTERN}/",
            certificate_activation_handler,
            name="certificate_activation_handler",
        ),
        re_path(
            r"^certificates/{}/(?P<certificate_id>\d+)/signatories/(?P<signatory_id>\d+)?$".format(
                settings.COURSE_KEY_PATTERN
            ),
            signatory_detail_handler,
            name="signatory_detail_handler",
        ),
        re_path(
            rf"^certificates/{settings.COURSE_KEY_PATTERN}/(?P<certificate_id>\d+)?$",
            certificates_detail_handler,
            name="certificates_detail_handler",
        ),
        re_path(
            rf"^certificates/{settings.COURSE_KEY_PATTERN}$",
            certificates_list_handler,
...
urlpatterns.append(
    path(
        "maintenance/",
        include("cms.djangoapps.maintenance.urls", namespace="maintenance"),
    )
)
...
urlpatterns.append(
    path(
        ...
        include(
            (
                "openedx.core.djangoapps.content.learning_sequences.urls",
                "learning_sequences",
            ),
...
if "openedx.testing.coverage_context_listener" in settings.INSTALLED_APPS:
    urlpatterns += [
        path(
            "coverage_context",
            include("openedx.testing.coverage_context_listener.urls"),
        )
    ]
