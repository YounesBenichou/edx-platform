...
from openedx.core.lib.teams_config import (
    TeamsConfig,
...
DEFAULT_COURSE_VISIBILITY_IN_CATALOG = getattr(
    settings, "DEFAULT_COURSE_VISIBILITY_IN_CATALOG", "both"
)
...
EXAM_SETTINGS_HTML_VIEW_ENABLED = getattr(settings, "FEATURES", {}).get(
    "ENABLE_EXAM_SETTINGS_HTML_VIEW", False
)
SPECIAL_EXAMS_ENABLED = getattr(settings, "FEATURES", {}).get(
...
edx_xml_parser = etree.XMLParser(
    dtd_validation=False, load_dtd=False, remove_comments=True, remove_blank_text=True
)
...
class Textbook:  # lint-amnesty, pylint: disable=missing-class-docstring
    ...
    def table_of_contents(self):
        ...
        try:
            ...
            Exception
        ) as err:  # lint-amnesty, pylint: disable=broad-except, unused-variable
            pass
    ...
    def __eq__(self, other):
        return self.title == other.title and self.book_url == other.book_url
...
def get_available_providers():  # lint-amnesty, pylint: disable=missing-function-docstring
    proctoring_backend_settings = getattr(settings, "PROCTORING_BACKENDS", {})

    available_providers = [
        provider for provider in proctoring_backend_settings if provider != "DEFAULT"
    ]
...
class CourseFields:  # lint-amnesty, pylint: disable=missing-class-docstring
    # djezzy-academy
    course_type = String(
        display_name=_("Type du Cours"),
        help=_("Definit le type du cours, Webinaire, hybride ou un Cours en ligne"),
    ...
    wiki_slug = String(
        help=_("Slug that points to the wiki for this course"), scope=Scope.content
    )
    enrollment_start = Date(
        help=_("Date that enrollment for this class is opened"), scope=Scope.settings
    )
    enrollment_end = Date(
        help=_("Date that enrollment for this class is closed"), scope=Scope.settings
    )
    start = Date(
    ...
    tabs = CourseTabList(
        help="List of tabs to enable in this course", scope=Scope.settings, default=[]
    )
    end_of_course_survey_url = String(
        display_name=_("Course Survey URL"),
        help=_(
            "Enter the URL for the end-of-course survey. If your course does not have a survey, enter null."
        ),
        scope=Scope.settings,
    ...
    course_wide_js = List(
        ...
        help=_(
            "Enter Javascript resource URLs you want to be loaded globally throughout the course pages."
        ),
        scope=Scope.settings,
    )
    ...
    course_wide_css = List(
        ...
        help=_(
            "Enter CSS resource URLs you want to be loaded globally throughout the course pages."
        ),
        scope=Scope.settings,
    )
...
class CourseBlock(
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            raise type(err)(
                f"{str(err)} For course: {str(self.id)}"
    ...
    def set_grading_policy(self, course_policy):
        ...
        if "GRADE_CUTOFFS" not in grading_policy:
            grading_policy["GRADE_CUTOFFS"] = CourseFields.grading_policy.default[
                "GRADE_CUTOFFS"
            ]
    ...
    def definition_to_xml(self, resource_fs):
        ...
        if self.textbooks:
            ...
            for (
                textbook
            ) in self.textbooks:  # lint-amnesty, pylint: disable=not-an-iterable
    ...
    def auto_cohort(self):
        ...
        return bool(self.cohort_config.get("auto_cohort", False))
    ...
    def start_date_is_still_default(self):
        ...
        return course_metadata_utils.course_start_date_is_default(
            self.start, self.advertised_start
        )
    ...
    def get_discussion_blackout_datetimes(self):
        ...
        try:
            ret = [
                ...
                for start, end in [
                    blackout_date for blackout_date in blackout_dates if blackout_date
                ]
            ]
            for blackout in ret:
    ...
    def video_pipeline_configured(self):
        ...
        return VideoUploadsEnabledByDefault.feature_enabled(course_id=self.id) or (
            self.video_upload_pipeline is not None
            and "course_video_upload_token" in self.video_upload_pipeline
        )
    ...
    def set_user_partitions_for_scheme(self, partitions, scheme):
        ...
        other_partitions = [
            p
            for p in self.user_partitions  # pylint: disable=access-member-before-definition
            if p.scheme != scheme
        ]
