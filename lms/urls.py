...
from lms.djangoapps.discussion.notification_prefs import (
    views as notification_prefs_views,
)
from lms.djangoapps.instructor.views import (
    instructor_dashboard as instructor_dashboard_views,
)
...
urlpatterns = [
    path(
        "", branding_views.index, name="root"
    ...
    path(
        "change_enrollment", student_views.change_enrollment, name="change_enrollment"
    ),
    ...
    path(
        ...
        include(
            (
                "common.djangoapps.entitlements.rest_api.urls",
    ...
    path(
        "api/demographics/",
        include("openedx.core.djangoapps.demographics.rest_api.urls"),
    ),
    ...
    path(
        "api/organizations/", include("organizations.urls", namespace="organizations")
    ),
    path(
        "catalog/",
        include(
            ("openedx.core.djangoapps.catalog.urls", "openedx.core.djangoapps.catalog"),
            namespace="catalog",
        ),
    ),
    # Update session view
    path(
        ...
        lang_pref_views.update_session_language,
    ...
    path(
        ...
        include(
            (
                "common.djangoapps.course_modes.rest_api.urls",
                "common.djangoapps.course_mods",
            ),
    ...
    path(
        "api/dashboard/",
        include("lms.djangoapps.learner_dashboard.api.urls", namespace="dashboard_api"),
    ),
    # Learner Home
    path(
        "api/learner_home/",
        include("lms.djangoapps.learner_home.urls", namespace="learner_home"),
    ),
    path(
        "api/experiments/",
        include(
    ...
    path(
        "api/discounts/",
        include(
            ("openedx.features.discounts.urls", "openedx.features.discounts"),
...
if settings.FEATURES.get("ENABLE_MOBILE_REST_API"):
    urlpatterns += [
        re_path(
            r"^api/mobile/(?P<api_version>v(2|1|0.5))/",
            include("lms.djangoapps.mobile_api.urls"),
        ),
    ]
...
if settings.FEATURES.get("ENABLE_OPENBADGES"):
    urlpatterns += [
        path(
            "api/badges/v1/",
            include(
                ("lms.djangoapps.badges.api.urls", "badges"), namespace="badges_api"
            ),
        ),
    ]
...
if settings.WIKI_ENABLED:
    ...
    urlpatterns += [
        ...
        path(
            "notify/",
            include((notify_url_patterns, notify_app_name), namespace="notify"),
        ),
        # These urls are for viewing the wiki in the context of a course. They should
        # never be returned by a reverse() so they come after the other url patterns
        re_path(
            rf"^courses/{settings.COURSE_ID_PATTERN}/course_wiki/?$",
            course_wiki_views.course_wiki_redirect,
            name="course_wiki",
        ),
        re_path(
            rf"^courses/{settings.COURSE_KEY_REGEX}/wiki/",
            include(
                (wiki_url_patterns, "course_wiki_do_not_reverse"),
...
urlpatterns += [
    ...
    re_path(
        rf"^videos/{settings.USAGE_KEY_PATTERN}$",
    ...
    path(
        ...
        include(
            (
                "openedx.core.djangoapps.xblock.rest_api.urls",
    ...
    re_path(
        r"^courses/{}/dates".format(settings.COURSE_ID_PATTERN),
    ...
    path(
        ...
        include(
            (
                "openedx.core.djangoapps.course_groups.urls",
    ...
    # Branding API
    path("api/branding/v1/", include("lms.djangoapps.branding.api_urls")),
    # Course experience
    re_path(
    ...
    re_path(
        rf"^courses/{settings.COURSE_ID_PATTERN}/",
...
if settings.FEATURES.get("ENABLE_TEAMS"):
    # Teams endpoints
    urlpatterns += [
        path("api/team/", include("lms.djangoapps.teams.api_urls")),
        re_path(
...
if settings.FEATURES.get("ENABLE_DISCUSSION_SERVICE"):
    urlpatterns += [
        path("api/discussion/", include("lms.djangoapps.discussion.rest_api.urls")),
        re_path(
...
if configuration_helpers.get_value(
    "ENABLE_BULK_ENROLLMENT_VIEW", settings.FEATURES.get("ENABLE_BULK_ENROLLMENT_VIEW")
):
    urlpatterns += [
...
urlpatterns += [
    ...
    path(
        "api/certificates/",
        include(
            ("lms.djangoapps.certificates.apis.urls", "lms.djangoapps.certificates"),
...
urlpatterns += [
    re_path(
        r"^xdomain_proxy.html$", cors_csrf_views.xdomain_proxy, name="xdomain_proxy"
    ),
]
...
if settings.FEATURES.get("CUSTOM_COURSES_EDX"):
    urlpatterns += [
        ...
        path(
            "api/ccx/",
            include(
                ("lms.djangoapps.ccx.api.urls", "lms.djangoapps.ccx"),
                namespace="ccx_api",
            ),
        ),
    ]
...
if settings.FEATURES.get("ENABLE_FINANCIAL_ASSISTANCE_FORM"):
    urlpatterns += [
        ...
        re_path(
            rf"financial-assistance/{settings.COURSE_ID_PATTERN}/apply/",
            courseware_views.financial_assistance_form,
            name="financial_assistance_form_v2",
        ),
        re_path(
            rf"financial-assistance/{settings.COURSE_ID_PATTERN}",
            courseware_views.financial_assistance,
...
urlpatterns += [
    ...
    path(
        "api/course_home/",
        include(("lms.djangoapps.course_home_api.urls", "course-home")),
    ),
    ...
    path(
        "api/course_home/v1/",
        include(("lms.djangoapps.course_home_api.urls", "course-home-v1")),
    ),
]
...
urlpatterns += [
    path(
        ...
        include("openedx.features.course_experience.api.v1.urls"),
    ),
]
...
if getattr(settings, "PROVIDER_STATES_URL", None):
    from lms.djangoapps.courseware.tests.pacts.views import (
        provider_state as courseware_xblock_handler_provider_state,
    )
...
urlpatterns += [
    path(
        ...
        include("lms.djangoapps.ora_staff_grader.urls", "ora-staff-grader"),
    ),
]
...
urlpatterns += [
    path(
        "api/instructor_task/", include("lms.djangoapps.instructor_task.rest_api.urls")
    ),
]
...
urlpatterns += [
    path(
        "api/mfe_config/v1",
        include(
            ("lms.djangoapps.mfe_config_api.urls", "lms.djangoapps.mfe_config_api"),
